"""Test suite P02-medium (ICP). pytest is missing -> __main__ runner.
...
def test_kabsch_recovers_known_transform():
    ...
    assert abs(np.linalg.det(R) - 1.0) < 1e-9        # a proper rotation, not a reflection


def test_kabsch_is_rotation_not_reflection():
    # even for degenerate/noisy data det(R)=+1 must hold
    rng = np.random.default_rng(3)
    P = rng.normal(size=(30, 3)); P[:, 2] *= 0.001    # almost planar
...
def test_icp_converges_clean():
    ...
    assert rotation_angle(R @ R_gt) < 2.0, "ICP should rotate the misalignment back"
    assert hist[-1] < 1e-3
...
def test_icp_basin_small_ok_large_fails():
    # a small misalignment converges, a very large one (150 deg) locks into a local minimum
    target = make_shape(1000, seed=0)
    # small: 20 deg -> success
    ...
    # large: 150 deg -> expected failure (the residual error stays large)
