"""Testsuite P03-final (Navigation). pytest fehlt -> __main__-Runner.
...
def test_world_collision():
    ...
    assert w.in_collision([-1.0, 5.0])            # ausserhalb der Grenzen
    # Strecke quer durch das Hindernis ist blockiert, aussen herum frei
    assert not w.segment_free([2.0, 5.0], [8.0, 5.0])
    assert w.segment_free([2.0, 0.5], [8.0, 0.5])
    # margin verschaerft den Test
...
def test_rrt_finds_collision_free_path():
    ...
    assert path is not None, "RRT sollte in dieser Welt einen Pfad finden"
    assert np.allclose(path[0], START) and np.allclose(path[-1], GOAL)
    for a, b in zip(path[:-1], path[1:]):
        assert w.segment_free(a, b, margin=0.3), "jede Kante muss kollisionsfrei sein"
...
def test_shortcut_shortens_and_stays_free():
    ...
    assert path_length(sm) < path_length(path)      # sollte echt kuerzer werden
...
def test_unicycle_and_wrap():
    ...
    # _wrap bildet auf ein 2pi-Intervall um 0 ab; +pi und -pi sind DERSELBE Winkel,
    # daher auf den Betrag testen.
...
def test_particle_filter_converges_to_true_pose():
    ...
    # Filter startet mit Versatz; ohne Bewegung, nur Messungen -> muss einrasten
