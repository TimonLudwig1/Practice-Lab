...
maximally permissive safe supervisor.  (Reference solution P02-medium)   Module 23 — Advanced Automation.
...
def find_deadlocks(net, states):
    ...
    return {m for m in states if len(enabled(net, m)) == 0 and m != net["marked"]}
...
def synthesize_supervisor(net, states, edges, forbidden):
    ...
    bad = set(forbidden)
    # grow `bad` by any state with an UNCONTROLLABLE transition into `bad` (badness unavoidable there)
    changed = True
    while changed:
        changed = False
        for (m, t, n) in edges:
            if n in bad and tname[t] in unc and m not in bad:
                bad.add(m)
                changed = True
    # the supervisor disables the CONTROLLABLE edges from safe states into `bad`
    disabled = [(m, tname[t]) for (m, t, n) in edges
                if n in bad and tname[t] not in unc and m not in bad]
    return bad, disabled
