...
P01. Here you implement the two CONTROLLERS (the `# TODO` spots) and study why cancelling the dynamics
turns a non-linear, coupled arm into a set of trivial linear systems. The solution is in solution/.
"""
import numpy as np
...
def pd_gravity_control(model, q, dq, qd, dqd, ddqd, Kp, Kd):
    ...
    """
    # TODO: tau = -Kp e - Kd edot + g(q), with e = q - qd, edot = dq - dqd
    #       (use model.gravity(q); this ignores ddqd)
...
def computed_torque_control(model, q, dq, qd, dqd, ddqd, Kp, Kd):
    ...
    """
    # TODO: v = ddqd - Kd edot - Kp e ; then
    #       tau = M(q) v + C(q,dq) dq + g(q)   (use model.mass_matrix / .coriolis / .gravity)
    raise NotImplementedError
