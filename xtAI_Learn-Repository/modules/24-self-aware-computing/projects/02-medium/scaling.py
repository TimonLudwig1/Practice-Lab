...
>>> YOUR TASK <<<
The M/M/c model, the workload, the MAPE-K loop and the metrics are given, and
`policy_control` is worked out as an example. You implement `min_replicas` (the model inversion)
and the two remaining policies. Solution in solution/.
...
def min_replicas(lam, mu, slo, c_max=40):
    ...
    """
    # TODO: return the smallest c in 1..c_max whose response_quantile_mmc(lam, mu, c) is below
    #       `slo`; if none qualifies, return c_max.
...
def policy_reactive(state, knob=0.75):
    ...
    # TODO: scale out (c+1) when util > knob, scale in (c-1) when util < knob - gap,
    #       otherwise keep c. The gap between the two thresholds IS the hysteresis.
    raise NotImplementedError
...
def policy_predictive(state, knob=1.10):
    ...
    # TODO: (1) forecast the load `delay` steps ahead by linear extrapolation of the smoothed
    #           load: forecast = ewma + (ewma - prev_ewma) * state["delay"]
    #       (2) return min_replicas(...) for that forecast times the safety margin `knob`
    #           (use state["mu"] and state["slo"]).
    raise NotImplementedError
