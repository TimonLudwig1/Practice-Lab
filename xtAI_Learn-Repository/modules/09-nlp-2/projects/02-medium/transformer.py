...
Contains:
  - sinusoidal_positional_encoding : fixed positional encoding   <- YOU (task 3)
  - TransformerEncoderBlock         : MHA + residual/LayerNorm ... <- YOU (task 3)
  - TransformerClassifier           : embedding + PE + blocks + pool + head (given)
"""
import math
import torch
...
def sinusoidal_positional_encoding(seq_len, d_model):
    r"""Fixed sinusoidal positional encoding (Vaswani eq. 5-6).
    ...
    TODO:
      - create pe as a zero matrix (seq_len, d_model).
      - pos = arange(seq_len) as a column (seq_len, 1).
      - div = exp(arange(0, d_model, 2) * (-log(10000)/d_model))   # (d_model/2,)
      - pe[:, 0::2] = sin(pos * div);  pe[:, 1::2] = cos(pos * div)
      - return pe
    """
    raise NotImplementedError("Task 3a: implement sinusoidal_positional_encoding")
...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        r"""TODO — two sublayers, each with a residual connection + LayerNorm:
          ...
          3. return x
        """
        raise NotImplementedError("Task 3b: implement TransformerEncoderBlock.forward")


class TransformerClassifier(nn.Module):
    """Encoder-only transformer for sequence classification (given)."""
