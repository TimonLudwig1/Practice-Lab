...
Als Nächstes zu bauen: **Modul 11 „Computer Vision"** — Skript + P01 + P02 FERTIG+GETESTET. Nächster Schritt: **Projekt 03-final** = Bildklassifikator EuroSAT, DREI Wege im Vergleich (KEINE Code-Vorgabe, README-Spec + loesung/): (a) kleines CNN **from scratch** (der „ohne pretrained"-Weg, klein/kurz halten!), (b) **Feature-Extraktion** (wie P02, eingefrorenes Backbone + Klassifikator), (c) **Fine-Tuning** (neuen Kopf + wenige Schichten mit kleiner LR kurz nachtrainieren). Plus Evaluation + Fehleranalyse. **HARDWARE: MacBook wird heiß → alles CPU/billig, kleine Subsets, wenige Epochen! Fine-Tuning nur Kopf o. letzte Schicht kurz.** EuroSAT-Infra aus P02 wiederverwendbar (data.py-Muster, torchvision datasets.EuroSAT, ~90MB, download 5s). Nach 11 → Modul 12 Image Processing.
...
| 11 | Computer Vision | projekte-in-arbeit | Notebook (basic), py (medium+final gepl.) | P01,P02 ✓ | Skript fertig. P01 ✓: Faltung+Sobel von Hand + gelernte ResNet-conv1-Filter. P02 ✓: Transfer/Feature-Extraktion (MobileNetV3 eingefroren) auf EuroSAT — Rohpixel 0.41 vs pretrained-Features 0.94, 5/5 Tests grün. P03-final offen (3 Wege: scratch-CNN vs Feature-Extr. vs Fine-Tuning) |
