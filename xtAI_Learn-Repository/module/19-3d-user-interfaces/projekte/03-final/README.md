...
> Abschlussprojekt. **Kein vorgegebener Code** — du entwirfst und baust die Studie selbst aus den Werkzeugen der Projekte P01 (Ray-Objekt-Schnitt) und P02 (angulares Zeigemodell). Referenzlösung in [`loesung/`](loesung/); **erst selbst versuchen**. Diese README ist die Spezifikation.
...
```bash
cd module/19-3d-user-interfaces/projekte/03-final
...
Vollständige Referenz in [`loesung/`](loesung/): `selection3d.py` (Generator + Techniken + Throughput), `stats_tools.py` (rank-biserial, Holm from scratch), `run.py` (drei Experimente + Plots), `test_selection3d.py` (8 Tests).
