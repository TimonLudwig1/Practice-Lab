...
```bash
cd module/18-multimodal-interfaces/projekte/02-medium
/Users/.../.venv/bin/python test_multimodal.py   # 7 Tests -> alle PASS
/Users/.../.venv/bin/python run.py                # beide Experimente + Plots
```

`run.py` druckt die Vergleichstabellen und legt zwei Plots in `ergebnisse/` ab (gitignored).
...
Vollständige, lauffähige Referenz in [`loesung/`](loesung/) (identische `run.py`/`test_multimodal.py`, plus ausgefüllte `multimodal.py`). Erst selbst versuchen!
