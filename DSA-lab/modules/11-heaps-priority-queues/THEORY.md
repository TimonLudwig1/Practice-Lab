...
An emergency room system doesn't just treat the person who's waiting the longest. An
operating system does not necessarily start the oldest process. And if a data pipeline
of millions of readings is looking for only the 20 largest outliers, it doesn't have to
sort all values completely. In all three situations, the central question is:

> Which element is **now** most important?

A **Priority Queue** formulates this question as an abstract interface. A **Binary
Heap** is the typical data structure that efficiently implements this interface. This
module consistently develops the heap in three steps: first the intuition, then concrete
state transitions, lastly invariants, correctness and complexity.

## Learning objectives

After this module you can:

- Priority Queue and Binary Heap conceptually separate,
- represent a min- or max-heap in an array,
- `push`, `peek`, `pop`, Sift-Up and Sift-Down,
- Implement bottom-up heapify and explain its runtime `O(n)`,
- Deriving Heap Sort and classifying its properties,
- Safely use Python's `heapq` with numbers and priorities,
- for top K choose the right heap type and the right heap size,
- k efficiently merge sorted consequences,
- maintain a running median with two heaps,
- Recognize priority queues in graphal algorithms and schedule systems.

---

# Part I — Intuition

## 1. Why a normal queue is not enough

A FIFO queue answers the question: "Who came first?" That's right for a cash line, but
wrong for time-critical jobs. A Priority Queue does not order by insertion time, but by
a **Priority Key**.

Suppose the smaller number means higher urgency:

| Arrival | Task | Priority |
|---:|---|---:|
| 1 | Create Backup | 8 |
| 2 | Check security alert | 1 |
| 3 | Export Report | 5 |

FIFO would deliver `Backup` first. A min priority queue returns `Sicherheitsalarm`
because `1` is the smallest priority key.

The abstract operations are:

- `push(item, priority)`: record an element,
- `peek()`: view the most important element,
- `pop()`: remove the most important element,
- optional `change_priority`: change a priority.

The Priority Queue does not require **how** these operations are implemented internally.
She's an Abstract Data Type. A Binary Heap is a concrete implementation.

## 2. Why not just sort it out?

Two simple implementations show the conflict:

| Representation | `push` | `peek` | `pop` of the minimum |
|---|---:|---:|---:|
| Unsorted list | `O(1)` | `O(n)` | `O(n)` |
| Sorted array | `O(n)` | `O(1)` | `O(1)` at the edge |
| Binary Min-Heap | `O(log n)` | `O(1)` | `O(log n)` |

An unsorted list postpones the cost of collection. A sorted array moves them to insert
because space has to be created. The heap requires less order: it only guarantees that
the most important element lies at the root. This leaves insertion and removal
logarithmic.
...
## 3. The decisive economy of the heap

A sorted array knows the complete order of all elements. A Min-Heap knows only local
relationships:

> Each parent node is smaller or equal to its children.

From this local rule only globally follows: The root is a minimum. There is no sorting
guarantee between siblings and between different subtrees. Precisely this incomplete
order saves work.

```text
Valid Min-Heap:          No sorted array:

          2                 [2, 5, 4, 12, 9, 8, 7]
       /     \
      5       4             5 is ahead of 4,
    /  \     / \            Nevertheless, the heap is valid.
   12   9   8   7
```

---

# Part II — Simulation: The Binary Heap as Array

## 4. Two invariants instead of pointers

A Binary Heap meets two independent invariants.

### Form invariant

The tree is **complete** (complete): All levels except the last are filled; the last
level is filled from left to right.

### Ordinary invariant

In the case of min-heap, each parent-child cante is `parent <= child`. In the case of
Max-Heap, `parent >= child` applies accordingly.

The formin variant allows a compact array display without node objects and without
pointer. In the case of zero-based indexing:
...
For the array `[2, 5, 4, 12, 9, 8, 7]`:

| Index `i` | Value | Parent index | Left index | Right index |
|---:|---:|---:|---:|---:|
| 0 | 2 | — | 1 | 2 |
| 1 | 5 | 0 | 3 | 4 |
| 2 | 4 | 0 | 5 | 6 |
...
Indices from `n // 2` have no children and are leaves.
...
## 5. Check the heap property

The array order does not have to be sorted. Only existing parent-child edges are tested:
...
Local comparisons suffice: Follow from any node the parent edges to the root. Each step
can only reduce or leave the value in the Min-Heap. Therefore, the root is no larger
than this node. This applies to every node, so the root is minimal globally.

## 6. Push and Sift-Up by Hand
...
The new value `3` is attached to the end of the array. This keeps the tree complete:
...
The only possible violation of order is on the path from the new leaf to the root.

### Step 2: Swap with the parent node

| Condition | Index | Parent | Comparison | Action |
|---|---:|---:|---|---|
| `[4,7,9,10,12,15,3]` | 6 | 2 | `3 < 9` | Exchange |
| `[4,7,3,10,12,15,9]` | 2 | 0 | `3 < 4` | Exchange |
| `[3,7,4,10,12,15,9]` | 0 | — | Root reached | Stop |

Sift-Up touches at most one node per layer. A complete tree with `n` elements has height
`floor(log2(n))`, which costs `push` `O(log n)`.
...
### Why Sift-Up Is Correct

Before the attachment, the old heap was valid. The new leaf does not yet have children;
only the edge to its parent can be injured. After an exchange, the edge down is correct:
the larger parent value occupies the old child position, below which already only larger
values lie. A new injury can only arise to the next parent edge. At the stop all edges
are valid again.

## 7. Peek: cheap but not remove

The root is at index `0`. Therefore, `peek` is `O(1)`:
...
Removing index `0` with normal list shift would cost `O(n)` and leave a gap at the root.
The heap uses its last element instead.

## 8. Pop and Sift-Down by hand
...
### Step 1: Secure minimum, get shape

We backup `2`, remove the last element `7` and put it to the root:

```text
[7, 5, 4, 12, 9, 8]
```

The tree remains complete. Only the order invariant may be injured on the root path
downwards.

### Step 2: Swap with the smaller child

| Condition | Index | Child values | Election | Action |
|---|---:|---|---:|---|
| `[7,5,4,12,9,8]` | 0 | `5`, `4` | `4` for index 2 | Exchange |
| `[4,5,7,12,9,8]` | 2 | `8`, none | `8` | `7 <= 8`, stop |

Why does the **smaller** child have to be chosen? An exchange with `5` would leave `4`
as a child of the new root `5` and immediately injured the invariant.
...
The special case with exactly one element is important: After `pop()` the list is empty,
so it is no longer allowed to write to index `0`.

## 9. Bottom-up-Heapify: make an array a heap

Any list is rarely a heap:

```text
[9, 4, 7, 1, -2, 6, 5]
```

All leaves are already valid heaps. For `n = 7`, the last inner node is `(n - 2) // 2 =
2`. We process inner nodes backwards:

| Start index | Condition after Sift-Down | Declaration |
|---:|---|---|
| 2 | `[9,4,5,1,-2,6,7]` | `7` swaps with child `5` |
| 1 | `[9,-2,5,1,4,6,7]` | `4` swaps with `-2` |
| 0 | `[-2,1,5,9,4,6,7]` | `9` drops over `1` |

When index `i` is edited, its child roots are already valid heaps. Sift-Down, together
with `i`, makes it a larger valid heap.
...
### Why Heapify doesn't cost `O(n log n)`

The coarse upper limit "`n` nodes times `log n`" is correct, but not sharp. Most nodes
are far below:

- approximately `n/2` nodes are leaves and cost `0` steps,
- approximately `n/4` nodes can drop `1` at most,
- approximately `n/8` nodes can decrease `2` planes at most,
- only very few nodes lie close to the root and can sink far.

The overall work is limited by the following series:
...
The weighted geometric series converges to a constant. Therefore, bottom-up heapify is
`O(n)`. The successive insertion of `n` elements is in the worst case `O(n log n)`.
...
Important: Various valid heap arrays can represent the same amount of value. A test
should therefore check invariant and multiquantity, not blindly expect a single layout.

---

# Part III — Formalisation

## 10. Definition and complexity

A **Binary Min-Heap** is a complete binary tree for which each node is `v` and each
direct child is `c`:
...
In the Max-Heap, the relation is reversed. From completeness follows height `h =
floor(log2 n)`. The standard operations are:

| Operation | Time | Add-on memory | Core idea |
|---|---:|---:|---|
| `peek` | `O(1)` | `O(1)` | Read Root |
| `push` | `O(log n)` | `O(1)` | End + Sift-Up |
| `pop` | `O(log n)` | `O(1)` | End to root + Sift-Down |
| Bottom-up Heapify | `O(n)` | `O(1)` | inner nodes backward |
| Find any value | `O(n)` | `O(1)` | No global order |
| Minimum in Max-Heap | `O(n)` | `O(1)` | Is somewhere in leaves |

### Grinding invariant of Sift-Up

Before each iteration, all heap edges are correct, except possibly the edge between
`child` and its parent. An exchange repairs this edge downwards; the only possible
injury moves one level upwards. There's no more injury when you're aborted.

### Grinding invariant of Sift-Down

Before each iteration, both child-part trees are valid heaps. Only the relationships
between `parent` and his children can be wrong. The exchange with the smaller child uses
the smallest of the three elements at the old parent position. The only possible injury
moves into the selected child part tree.

## 11. Heap Sort

A Max-Heap puts the largest element to the root. For ascending sorting, this root is
replaced with the last element of the active area. After that, the maximum is at its
final position. Sift-Down only repairs the reduced prefix.
...
Heap Sort requires `O(n)` for Heapify and then `n - 1` for each of `O(log n)`: total
`O(n log n)` in Best, Average and Worst Case. The array-based variant works with `O(1)`
additional memory. But it is not stable: the same key can lose its relative order when
swapping.

## 12. Python: `heapq`

Pythons default module `heapq` works directly on normal lists and provides a min-heap.
The most important functions are:

- `heapq.heapify(list)` changes the list to `O(n)`,
- `heapq.heappush(heap, item)` inserts:
- `heapq.heappop(heap)` removes the minimum,
- `heapq.heappushpop(heap, item)` combines push and pop efficiently,
- `heapq.heapreplace(heap, item)` pops first and then pushes; the heap does not have to
  be empty,
- `heapq.nsmallest` and `heapq.nlargest` are comfort functions for small `k`.
...
For numbers, the sign is usually negated. The smallest negative element corresponds to
the largest original value:
...
Negation is only suitable for numerical priorities. For more complex objects, a
comparable key is placed in a tuple or a matching wrapper is defined.

### Tuple and Stable Tie-Breaker

Tuples are compared lexicographically: first priority, then the second field and so on.
If two priorities are equal and the utility objects are not comparable, `(priority,
task)` creates a `TypeError`. A monotonic counter solves the problem and receives FIFO
order under the same priorities.
...
### Amend priorities retrospectively

`heapq` does not know any direct decrease key operation. In many algorithms, a new entry
with better priority is pushed instead. In pop, outdated entries are skipped. This
**lacy deletion** pattern is often easier to keep current than the position of an
element in the heap.
...
## 13. Top-K: Keeping the heap small

The most important design question is not only "Min- or Max-Heap?", but also:

> Which elements should remain in the heap, and which of them should easily
> can be thrown out?

### K largest elements

Hold a **min-heap of size k**. The root is the smallest element under the best k so far
and thus the current limit. A new value replaces the root only if it is larger.

Example for `k = 3` and stream `5, 1, 9, 3, 14, 8`:

| Value | Heap of the so far 3 largest | Decision |
|---:|---|---|
| 5 | `[5]` | record |
| 1 | `[1,5]` | record |
| 9 | `[1,5,9]` | record |
| 3 | `[3,5,9]` | `3 > 1`, replace limit |
| 14 | `[5,14,9]` | `14 > 3`, replace limit |
| 8 | `[8,14,9]` | `8 > 5`, replace limit |
...
Time: `O(n log k)`, memory: `O(k)`. If `k` is much smaller than `n`, this is more
attractive than complete sorting in `O(n log n)`.

For the **k smallest** elements, the logic is mirrored: a max-heap of the size `k` holds
the currently largest of the small elements at the root.

### Top-K most common values

First, a hash map generates the frequencies. After that, a min-heap only holds the `k`
best `(frequency, value)` pairs. The pattern combines hashing and heap.
...
## 14. k-Way-Merge: Always choose the smallest head

The following are given: `k` already sorted episodes with total `n` Elements. A naïve
run could be used for every issue `k` Browse current heads: `O(nk)`. A Min-Heap only
saves the current head of each non-empty sequence.
...
Heap starts with (1,A), (2,B), (4,C).
Pop (1,A), then Push (7,A).
Pop (2,B), then push (3,B).
Pop (3,B), then Push (11,B).
...
```

The heap contains a maximum of `k` entries. Each of the `n` outputs causes a pop and
usually a push: time `O(n log k)`, additional memory `O(k)` without output.
...
The `sequence_index` is also Tie-Breaker if two head values are equal. The pattern is
the basis of external sorting, merge phases in databases and merge time-sorted event
streams.

## 15. Running median with two heaps

The median of a stream should be available after each new number. Complete sorting by
each insert would be expensive. The data are divided into two halves:

- `lower`: Max-heap of the smaller half, in python by negated values,
- `upper`: Min-heap of the larger half.

Two invariants shall apply:

1. **Order:** Each element in `lower` is `<=` each element in `upper`.
2. **Balance:** The sizes differ by one at most; here `lower` exactly one element may
   have more.

Simulation for `5, 2, 10, 4`:

| New | Lower (content) | Upper | Median |
|---:|---|---|---:|
| 5 | `[5]` | `[]` | 5 |
| 2 | `[2]` | `[5]` | 3,5 |
| 10 | `[5,2]` | `[10]` | 5 |
...
Each insert needs `O(log n)`, the median itself `O(1)`, the memory `O(n)`. The principle
is generalised to online statistics, but quantiles need other or extended data
structures.

---

# Part V — Priority Queues in Algorithms and Systems

## 16. Dijkstra anticipation

When searching for Dijkstra for the shortest way, the priority of a node is its best
known distance from the start. The next processing always takes the node with the
smallest preliminary distance.

```text
Priority queue: (0, start), (7, A), (12, B),...
Pop always returns the currently next unfinished node.
```

If the algorithm finds a better distance, a new `(distance, node)` entry is often
pushed. A later popped entry is discarded if its distance no longer corresponds to the
stored best distance. That's the same Lazy Deletion pattern as with changing priorities.

With Adjacency List and Binary Heap typically results in `O((V + E) log V)`, often
written as `O(E log V)`. Module 13 develops the complete algorithm.

## 17. Scheduler: Priority is a product decision

A scheduler can select jobs according to urgency, deadline, remaining time or a
combination. The heap makes the selection efficient, but does not decide whether the
Priority Policy is fair.

A pure priority scheduler can generate **Starvation**: When continuously high-priority
jobs arrive, a low job waits indefinitely. Possible countermeasures are:

- **Aging:** Priority increases with waiting time,
- Quotas per priority class,
- Round-robin within the same priority,
- Deadline-based keys,
- limited Burst's highly prioritized work.

The Tie-Breaker in the tuple is therefore not only technical. `(priority,
arrival_counter, job)` defines FIFO behavior within a class and makes simulation
deterministic.

## 18. Data Science references

Heaps often appear indirectly in data science pipelines:

- Top K features, anomalies, search results or most common categories,
- Merge pre-sorted Shards or log streams,
- Beam Search and Best-First Search,
- Nearest-Neighbor procedures with a limited number of candidates;
- streaming media and running ranking statistics,
- Prioritize expensive experiments or data quality alerts.

The common structure is that there is a large or growing quantity, but per step only the
best element or a small limit is interested.

---

# Part VI — Traps and design decisions

## 19. Common Mistakes in Thought

### "A heap is sorted"

Wrong. Only parent-child relationships are arranged. Inorder traversal or array order do
not deliver a sorted sequence. Repeated popping, on the other hand, provides elements in
priority order.

### "I can search for any value binary"

Wrong. From `target > root` does not follow in which subtree the target lies. Any search
is in the worst case `O(n)`.

### "I need a Max-Heap for k's greatest values"

Not if the heap is to hold only `k` elements. Then the **badest of the currently best**
elements must be quickly removable: a min-heap.

### "Heapify is repeated pushing"

Both produce a heap, but only bottom-up heapify guarantees linear construction. The
result layout may be different.

### "The same priority automatically means stable order"

A heap is not stable. For stability, the arrival order must be explicitly part of the
comparison key.

## 20. Mutations, duplicates and special numbers

- Duplicates do not violate the heap invariant; `<=` allows equality.
- Changing an element in the array directly can break the invariant. After priority
  improvement, Sift-Down is required depending on the heap sift-up, after deterioration.
- Mutable objects should not determine the comparison themselves. An unchangeable
  priority stub is safer.
- `float('nan')` has no normal total order. Comparisons with NaN are almost always
  `False` and can destroy heap assumptions. Data should be validated before insertion.
- In the case of negated Max-Heap-Keys, the sign must be consistently transformed back
  when reading and writing.

## 21. Which structure is the right one?

| Problem | Suitable structure | Reasons |
|---|---|---|
| Always taking the smallest element | Min-heap | Root is minimum |
| Always taking the largest element | Max-Heap | Root is maximum |
| Test any membership quickly | Hash Set | Heap search is linear |
| Iterate all values assorted | sorted array / tree | Heap is only partially ordered |
| K largest from large stream | Min-heap of size `k` | Limit is at root |
| Median of a stream | Max-Heap + Min-Heap | both mean limits `O(1)` |
| FIFO processing | Queue | Unnecessary priority order |

---

# Part VII — Transfer and self-control

## 22. Decision-making recipe for heap problems

In this order, put questions:

1. Does an extreme value have to be delivered repeatedly?
2. Is the wanted extremum minimum or maximum?
3. Should the heap hold all elements or only `k` candidates?
4. What element must lie at the root of the problem so that a bad candidate can be
   easily removed?
5. Are there equal priorities, non-comparable payloads or stability?
6. Can priorities become obsolete afterwards?
7. Is Lazy Deletion enough or is an index map needed for real update?

## 23. Control questions

1. Why is `[1, 4, 2, 9, 7, 3]` a valid min-heap, although `4 > 2`?
2. Which edges can be injured right after a push?
3. Why does Sift-Down trade with the smaller child at Min-Heap?
4. Which index is the last inner node for `n` elements?
5. Why is Heapify total `O(n)` despite individual `O(log n)`-sift-downs?
6. Which heap do you use for the 100 largest values of a billion-dollar stream?
7. What does a k-Way-Merge entry need to save out of value?
8. Which two invariants does the running media algorithm hold?
9. Why can `(priority, task)` fail in `heapq` with the same priority?
10. Why does a heap not solve a scheduler's fairness problem?

### Short Answers

1. `4` and `2` are siblings; only parent-child edges are arranged.
2. Only the path from the new leaf to the root.
3. Otherwise, the smaller sibling could stay under a larger parent.
4. `(n - 2) // 2` for `n >= 2`.
5. Almost all nodes lie below and can only sink very briefly.
6. A Min-Heap of size 100.
7. Order of origin and position, so that the successor can be postponed.
8. Order between the halves and size balance.
9. Python then tries to compare the potentially uncomparable tasks; a counter serves as
   a tie-breaker.
10. The heap only efficiently executes the specified priority policy.

## 24. Compact overall overview
...
Priority Queue (ADT)
    ...
    +-- Binary Heap (implementation)
            |
            +-- complete-tree invariant -> compact array
            +-- heap order invariant    -> Extremum at root
            +-- push                    -> append + sift-up
            +-- pop                     -> last-to-root + sift-down
            +-- heapify                 -> inner node bottom-up, O(n)
            |
            +-- Patterns
                    +-- Top K            -> Limited counter-heap
                    +-- k-Way-Merge      -> Heap of current heads
                    +-- Running Median   -> Max-Heap + Min-Heap
                    +-- Dijkstra         -> smallest known distance
                    `-- Scheduler        -> best priority key
```

## 25. Outlook for the projects

In the project **01-basic**, a Min-Heap is completely self-built, tested against `heapq`
property and extended to Heap Sort. **02-medium** isolates the three most important
patterns Top-K, k-Way-Merge and Running Median. **03-final** uses a priority queue in a
reproducible job scheduler and compares its waiting times per priority class with FIFO.

The central insight for all three projects is:

> A heap doesn't sort everything. He invests just as much order that the
> the next relevant extreme value is efficiently available.
